...
export let API_BASE_URL = process.env.NEXT_PUBLIC_API_URL;

if (process.env.NODE_ENV === "production") {
  if (!API_BASE_URL || API_BASE_URL.includes("127.0.0.1") || API_BASE_URL.includes("localhost")) {
    API_BASE_URL = "https://api.vqtmanagement.com/api";
  }
} else {
  if (!API_BASE_URL) {
...
class ApiService {
  constructor() {
    ...
    if (typeof window !== "undefined" && (!base || base.includes("localhost") || base.includes("127.0.0.1"))) {
      // In browser environment if base is not an absolute domain, default to production domain if on production host
      if (window.location.hostname !== "localhost" && window.location.hostname !== "127.0.0.1") {
        base = process.env.NEXT_PUBLIC_API_URL || "https://api.vqtmanagement.com/api";
      }
    }
    if (base && !base.startsWith("http://") && !base.startsWith("https://")) {
      base = `https://${base.replace(/^\/+/, "")}`;
    }
    this.baseURL = base || "http://127.0.0.1:8000/api";
    this.storageURL = this.baseURL.replace(/\/api\/?$/, "") + "/storage";
    this.token = null;
  }
  ...
  async downloadAgreementPdf(uuid) {
    const url = `${this.baseURL}/client-agreement/download/${uuid}`;
    ...
    const contentDisposition = response.headers.get("Content-Disposition");
    let filename = "Client_Agreement.pdf";
    if (contentDisposition) {
      const filenameMatch = contentDisposition.match(/filename="?([^"]+)"?/);
      if (filenameMatch) {
        filename = filenameMatch[1];
  ...
  async markReadyForCounsellorSelection(clientId) {
    return this.request(`/clients/${clientId}/ready-for-counsellor-selection`, {
      method: "POST",
    });
  }
  ...
  async getTCSlotBookings(id) {
    return this.request(`/training-counsellors/${id}/slot-bookings`);
  }
  ...
  async createTrainingCounsellor(data, files = {}) {
  ...
  async uploadTrainingCounsellorPhoto(id, fileOrFormData) {
    const formData =
      fileOrFormData instanceof FormData ? fileOrFormData : new FormData();
    if (!(fileOrFormData instanceof FormData)) {
      formData.append("photo", fileOrFormData);
    }
    return this.request(`/training-counsellors/${id}/upload-photo`, {
      method: "POST",
      body: formData,
    });
  }

  async deleteTrainingCounsellorPhoto(id) {
    return this.request(`/training-counsellors/${id}/delete-photo`, {
      method: "DELETE",
    });
  }

  async getTcHolidays(id) {
    return this.request(`/training-counsellors/${id}/holidays`);
  }

  async addTcHoliday(id, data) {
    return this.request(`/training-counsellors/${id}/holidays`, {
      method: "POST",
      body: JSON.stringify(data),
    });
  ...
  async deleteTcHoliday(id, holidayId) {
    return this.request(`/training-counsellors/${id}/holidays/${holidayId}`, {
      method: "DELETE",
    });
  }

  async getTcCaseload(id) {
    return this.request(`/training-counsellors/${id}/caseload`);
  }

  // Counsellor time-off requests
  async getMyHolidays() {
    return this.request("/counsellor/holidays");
  }
  ...
  // Admin: time-off request queue
  async getPendingHolidays() {
    return this.request("/training-counsellor-holidays/pending");
  }

  async approveHoliday(id) {
    return this.request(`/training-counsellor-holidays/${id}/approve`, {
      method: "PATCH",
    });
  }

  async rejectHoliday(id) {
    return this.request(`/training-counsellor-holidays/${id}/reject`, {
      method: "PATCH",
    });
  }
  ...
  // Trainee counsellor conduct/progress events (admin-logged)
  async getTcConductEvents(id) {
    return this.request(`/training-counsellors/${id}/conduct-events`);
  }

  async logTcConductEvent(id, data) {
    return this.request(`/training-counsellors/${id}/conduct-events`, {
      method: "POST",
      body: JSON.stringify(data),
    });
  ...
  // Counsellor portal: own conduct/progress events
  async getMyConductEvents() {
    return this.request("/counsellor/conduct-events");
  }
  ...
  async bulkPortalInvite(ids) {
    return this.request("/training-counsellors/bulk-portal-invite", {
      method: "POST",
  ...
  async getPlacementResponse(token) {
    return this.request(`/trainee-placement-response/${token}`);
  }

  async submitPlacementResponse(token, { placementAccepted, inductionRsvp }) {
    return this.request(`/trainee-placement-response/${token}`, {
      method: "POST",
      body: JSON.stringify({
        placement_accepted: placementAccepted,
        induction_rsvp: inductionRsvp,
      }),
    });
  }
  ...
  async deleteConversation(peerType, peerId) {
    return this.request(`/messages/conversations/${peerType}/${peerId}`, {
      method: "DELETE",
    });
  }
  ...
  async restoreMessage(id) {
    return this.request(`/messages/${id}/restore`, {
      method: "POST",
    });
  }
  ...
  async getMessageStaffList() {
    return this.request("/messages/staff-list");
  }
  ...
  async uploadCounsellorPhoto(fileOrFormData) {
    const formData =
      fileOrFormData instanceof FormData ? fileOrFormData : new FormData();
    if (!(fileOrFormData instanceof FormData)) {
      formData.append("photo", fileOrFormData);
    }
    return this.request("/counsellor/upload-photo", {
      method: "POST",
      body: formData,
    });
  }

  async deleteCounsellorPhoto() {
    return this.request("/counsellor/delete-photo", {
      method: "DELETE",
    });
  }

  async updateConsultationDelegationPreferences(data) {
    return this.request("/counsellor/consultation-availability-toggles", {
      method: "PUT",
      body: data,
    });
  }
  ...
  async shareFolder(folderId, shares) {
    return this.request(`/folders/${folderId}/share`, {
      method: "POST",
      body: { shares },
    });
  }
  ...
  async getSessionNote(id) {
    return this.request(`/session-notes/${id}`);
  }
  ...
  async getNextBlockSlots(clientUuid) {
    return this.requestPublic(
      `/client-booking/${clientUuid}/next-block-slots`,
    );
  }

  async getAvailableCounsellors(clientUuid) {
    return this.requestPublic(
      `/client-booking/${clientUuid}/available-counsellors`,
    );
  }

  async chooseCounsellor(clientUuid, tcUuid) {
    return this.requestPublic(
      `/client-booking/${clientUuid}/choose-counsellor`,
      {
        method: "POST",
        body: JSON.stringify({ tc_uuid: tcUuid }),
      },
    );
  }
  ...
  // Intake Form submissions (Public)
  async submitClientIntake(data) {
    return this.requestPublic("/client-intake", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }

  // Mid Range / Coaching & Counselling intake (Public)
  async getFilteredCounsellors(data) {
    return this.requestPublic("/mid-range-intake/filtered-counsellors", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }

  // Qualified Counsellor Form — public prefill by UUID (no auth required)
  async getQualifiedCounsellorPrefill(uuid) {
    return this.requestPublic(`/qualified-counsellor/prefill/${uuid}`);
  }

  async getCounsellorConsultationAvailability(tcUuid) {
    return this.requestPublic(
      `/mid-range-intake/consultation-availability/${tcUuid}`,
    );
  }

  async submitTCIntake(data) {
    // If data is FormData (for file uploads), requestPublic handles it
    return this.requestPublic("/tc-intake", {
      method: "POST",
  ...
  async getConsultationSlots(type) {
    return this.request(type ? `/consultation-slots?type=${type}` : "/consultation-slots");
  }
  ...
  async createConsultationSlotRange(data) {
    return this.request("/consultation-slots/range", {
      method: "POST",
      body: data,
    });
  }

  async createRecurringConsultationSlots(data) {
    return this.request("/consultation-slots/recurring", {
      method: "POST",
      body: data,
    });
  }

  async createBulkConsultationSlots(data) {
    return this.request("/consultation-slots/bulk", {
      method: "POST",
      body: data,
    });
  }

  async updateConsultationSlot(id, data) {
  ...
  // Matching Algorithm Settings endpoints
  async getMatchingAlgorithmSettings() {
    return this.request("/matching-algorithm-settings");
  }

  async updateMatchingAlgorithmSettings(data) {
    return this.request("/matching-algorithm-settings", {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  // 4-Way Agreement Document (admin settings)
  async getFourWayAgreementStatus() {
    return this.request("/company-settings/four-way-agreement/status");
  }

  async uploadFourWayAgreement(file) {
    const formData = new FormData();
    formData.append("document", file);
    return this.request("/company-settings/four-way-agreement", {
      method: "POST",
      body: formData,
    });
  }

  async deleteFourWayAgreement() {
    return this.request("/company-settings/four-way-agreement", {
      method: "DELETE",
    });
  }

  // PSG Attendance System
  async getPsgSessions() {
    return this.request("/counsellor/psg-sessions");
  }

  async getGroupSessions(groupId) {
    return this.request(`/attendance-groups/${groupId}/psg-logs`);
  }

  async getPublicPsgForm(token) {
    return this.requestPublic(`/public/psg/${token}`);
  }

  async submitPublicPsgSession(token, data) {
    return this.requestPublic(`/public/psg/${token}`, {
      method: "POST",
      body: JSON.stringify(data),
    });
  ...
  async toggleGroupStatus(groupId) {
    return this.request(`/attendance-groups/${groupId}/toggle-status`, {
      method: "PATCH",
    });
  }

  async getGroupDiscussions(groupId) {
    return this.request(`/attendance-groups/${groupId}/discussions`);
  }

  async postGroupDiscussion(groupId, message) {
    return this.request(`/attendance-groups/${groupId}/discussions`, {
      method: "POST",
      body: { message },
    });
  ...
  // Public request method (no auth token) — with timeout and retry on network failure
