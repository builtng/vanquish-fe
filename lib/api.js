    ...
    const makeRequest = async (attempt = 0) => {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          throw new ApiError('Too many requests. Please wait a moment and try again.', 429, data);
        }
        
        throw new ApiError(data.message || data.error || `Server error: ${response.status}`, response.status, data);
      }
