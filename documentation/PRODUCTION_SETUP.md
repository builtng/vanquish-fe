...
### 8. Real-time (WebSockets) Setup

This application uses **Pusher** for real-time broadcasting.

#### Pusher Configuration

Ensure you have a Pusher account and have created an app. Update your `.env` with the credentials:
...
On shared hosting environments where long-running processes (like Laravel Reverb or WebSockets) are not allowed, Pusher is the recommended solution as it is a hosted service. Ensure your `QUEUE_CONNECTION` is set to `sync` if you cannot run a queue worker.
