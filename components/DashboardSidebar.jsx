...
export default function DashboardSidebar() {
  ...
  const transformConsultationData = (consultations) => {
    ...
    return consultations.filter(c => c).map((consultation) => ({
      id: consultation.id,
  ...
  useEffect(() => {
    ...
    const handleRefresh = () => {
      ...
      // but let's just bypass it by setting lastFetchRef to 0 momentarily
      lastFetchRef.current = 0;
      fetchSidebarData();
    };
    window.addEventListener("refresh-sidebar-data", handleRefresh);
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (menuPrivileges && menuPrivileges.length > 0) {
      // If it's a group, show if the group itself is allowed OR any sub-item is allowed
      if (item.subItems) {
