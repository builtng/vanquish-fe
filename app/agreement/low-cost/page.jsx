...
function LowCostAgreementContent() {
  ...
  const validateForm = () => {
    ...
    if (!formData.caseStudyConsent)
      newErrors.caseStudyConsent = "Please select yes or no";
    ...
    if (!formData.gpPracticePhone.trim())
      newErrors.gpPracticePhone = "GP practice phone is required";
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/client-agreement/submit`,
        {
          ...
          body: JSON.stringify({
            ...
            case_study_consent: formData.caseStudyConsent,
