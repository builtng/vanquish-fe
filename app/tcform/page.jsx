...
import { User, Briefcase, Calendar, AlertTriangle, CheckCircle, Save } from 'lucide-react';
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            Thank you for completing your profile. Our admin team will review your information and get back to you shortly.
          </p>
          <button
            onClick={() => {
              setSubmitted(false);
              setCurrentStep(1);
              setFormData({
                fullName: '', email: '', phone: '', age: '', gender: '', ethnicity: '',
                sexualOrientation: '', lgbtqSpecialist: false, modalities: [], experienceYears: '',
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <div className="md:col-span-2">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => handleInputChange('lgbtqSpecialist', e.target.checked)}
                      className="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-600"
                    />
                    <span className="text-sm font-medium text-gray-700">
                      I am an LGBTQ+ specialist (have training/expertise in LGBTQ+ issues)
                    </span>
                  </label>
                </div>
              </div>
                ...
                <div className="border border-gray-300 rounded-lg p-4 max-h-64 overflow-y-auto">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {modalities.map((modality) => (
                      <label key={modality} className="flex items-start gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                        <input
                          type="checkbox"
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">Your Availability</h2>
                <p className="text-sm text-gray-600 mb-4">
                  Select all days and time blocks when you're available for sessions
                </p>
              </div>
              
              <div className="border border-gray-300 rounded-lg overflow-hidden">
                <div className="grid grid-cols-4 bg-gray-50 border-b border-gray-300">
                  <div className="p-3 font-medium text-sm text-gray-700 border-r border-gray-300">Day</div>
                  <div className="p-3 font-medium text-sm text-gray-700 text-center border-r border-gray-300">Morning</div>
                  <div className="p-3 font-medium text-sm text-gray-700 text-center border-r border-gray-300">Afternoon</div>
                  <div className="p-3 font-medium text-sm text-gray-700 text-center">Evening</div>
                </div>
                {days.map((day) => (
                  <div key={day} className="grid grid-cols-4 border-b border-gray-200 last:border-b-0 hover:bg-gray-50">
                    <div className="p-3 font-medium text-sm text-gray-900 capitalize border-r border-gray-200">
                      {day}
                    </div>
                    {timeBlocks.map((timeBlock) => (
                      <div key={timeBlock} className="p-3 flex items-center justify-center border-r border-gray-200 last:border-r-0">
                        <label className="cursor-pointer">
                          <input
                            type="checkbox"
                            checked={formData.availability[day].includes(timeBlock)}
                            onChange={() => handleAvailabilityToggle(day, timeBlock)}
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">Topics You're Not Ready to Handle</h2>
                <p className="text-sm text-gray-600 mb-4">
                  Select any topics you currently feel unprepared to work with. This ensures client safety and your comfort.
                </p>
              </div>
              ...
              <div className="border border-gray-300 rounded-lg p-4 max-h-96 overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {sensitiveTopics.map((topic) => (
                    <label key={topic} className="flex items-start gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                      <input
                        type="checkbox"
              ...
              {formData.notReadyFor.length > 0 && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  <p className="text-sm text-orange-800">
                    You've marked <strong>{formData.notReadyFor.length} topic(s)</strong> as areas you're not ready to handle. 
                    The system will flag any clients with these issues for admin review.
                  </p>
                </div>
              )}
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <p className="text-sm text-gray-700">
                  <strong>Important:</strong> You can update this list anytime as you gain more experience and training.
                </p>
              </div>
            </div>
          )}
