...
export default function UsersPage() {
  ...
  const [userCount, setUserCount] = useState({ count: 0, max_users: 3, can_add_more: true });
  ...
  const handleAddUser = () => {
    if (!userCount.can_add_more) {
      showError(`Maximum number of users (${userCount.max_users}) has been reached.`);
      return;
    }
    setFormData({
      name: '',
  ...
  const saveUser = async () => {
    ...
    if (!namePattern.test(trimmedName)) {
      showError('Name can only contain letters, spaces, hyphens (-), apostrophes (\'), and periods (.)');
      return;
    }

    if (showAddModal && (!formData.password || !formData.password_confirmation)) {
  ...
  if (!isAdmin) {
    return (
      <DashboardLayout>
        <div className="flex-1 flex items-center justify-center bg-gray-50 dark:bg-[var(--background)]">
          <div className="text-center max-w-md mx-auto p-8">
            <AlertCircle className="w-16 h-16 text-red-500 dark:text-red-400 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-900 dark:text-[var(--text-primary)] mb-2">Access Denied</h2>
            ...
            <p className="text-sm text-gray-500 dark:text-[var(--text-tertiary)]">
              Your current role: <span className="font-semibold text-gray-900 dark:text-[var(--text-primary)]">{authUser?.role || 'Unknown'}</span>
            </p>
          </div>
        </div>
      </DashboardLayout>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-[var(--text-secondary)] mt-1">Manage system users and access</p>
          </div>
        </DashboardHeader>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-[var(--background)]">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-gray-500 dark:text-[var(--text-secondary)]">Loading users...</div>
            </div>
          ) : users.length === 0 ? (
            <div className="bg-white dark:bg-[var(--card-bg)] rounded-lg border border-gray-200 dark:border-[var(--card-border)] p-12 text-center">
              ...
              <p className="text-gray-600 dark:text-[var(--text-secondary)]">No users found</p>
            </div>
          ) : (
            <div className="bg-white dark:bg-[var(--card-bg)] rounded-lg border border-gray-200 dark:border-[var(--card-border)] overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-[var(--card-border)]">
                ...
                <tbody className="bg-white dark:bg-[var(--card-bg)] divide-y divide-gray-200 dark:divide-[var(--card-border)]">
                  {users.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-[var(--hover-bg)]">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-[var(--text-secondary)]">
                        {user.two_factor_enabled ? (
                          ...
                          <span className="text-gray-400 dark:text-[var(--text-tertiary)]">Disabled</span>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-[var(--text-secondary)]">
                        {user.created_at
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-[var(--card-bg)] rounded-lg shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-[var(--text-primary)] mb-2">Role *</label>
                  <select
                    value={formData.role}
                ...
                {formData.password && formData.password_confirmation &&
                  formData.password !== formData.password_confirmation && (
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
                      <p className="text-sm text-red-600 dark:text-red-300">Passwords do not match</p>
                    </div>
                  )}
